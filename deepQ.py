...
class DQN:
    ...
    def train(self, n_episodes, batch_size,
              epsilon_start, epsilon_stop, epsilon_anneal_episodes,
              gamma, target_net_update_freq, polyak_average, 
              tau, checkpoint_freq):

        self.replay_buffer.reset()
        epsilon = epsilon_start
        epsilon_decay = math.pow(epsilon_stop / epsilon_start, 1 / epsilon_anneal_episodes)
...
class double_DQN:
    # double q learning, decoupling action selection from action evaluation
    def __init__(self, env_id, 
                 state_shape, n_actions, lr,
    ...
    def update_q(self, batch_size, gamma, global_frame, track_q = False):
        ...
        with torch.no_grad():
            future_q_online = self.q_network(next_state_b)
            argmax_online_q= torch.argmax(future_q_online, dim=1, keepdim=True)
            evaluation_q_values = self.target_network(next_state_b)
            selected_q_values = evaluation_q_values.gather(dim=1, index=argmax_online_q) 

        # everything must be shape (1, dim)
        future_q = selected_q_values
        target_q = reward_b.view(-1, 1) + gamma * future_q * (1-done_b.view(-1, 1))
